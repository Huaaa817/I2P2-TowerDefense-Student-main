...
Enemy::Enemy(std::string img, float x, float y, float radius, float speed, float hp, int money, int score) :
	Engine::Sprite(img, x, y), speed(speed), hp(hp), money(money), score(score) {
	CollisionRadius = radius;
	reachEndTime = 0;
}
void Enemy::Hit_die(float damage) {
	hp -= damage;
	if (hp <= 0) {
		OnExplode();
