		...
		if (enemyWaveData.empty()) {
			if (EnemyGroup->GetObjects().empty()) {
				...
				//if(!written) {
					...
					datetime = std::ctime(&now_time);
					datetime.pop_back();
					/*
					std::string filename = "../Resource/scoreboard.txt";
					std::ofstream outfile(filename, std::ios::app);
					outfile << std::string("nobody") + " " + std::to_string(score) + " " + datetime << std::endl;
					outfile.flush();
					outfile.close();
					written=true;
					*/
...
std::string PlayScene::GetTime() const {
	return datetime;
}
...
void PlayScene::EarnScore(int score) {
	...
	//Engine::LOG(Engine::DEBUGGING) <<this->score;
}
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
		...
		for(auto& dir : PlayScene::directions) {
			int x = p.x + dir.x;
			int y = p.y + dir.y;
			if (x < 0 || x >= PlayScene::MapWidth || y < 0 || y >= PlayScene::MapHeight || mapState[y][x] != TILE_DIRT || map[y][x]!=-1)
				continue;
